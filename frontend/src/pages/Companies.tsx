...
import { getCompanies, updateCompanyItem, deleteCompanyItem } from '../api/companies'
...
const REQUIRED_HEADERS = ['Company','Sector','Contacted person','Methods to reach out','Status','Comments'] as const
...
export default function Companies() {
  ...
  function resetColumnFilters(col: string){ setFilters(prev => { const n = { ...prev }; delete n[col]; return n }) }
  ...
  async function onDelete(id: string){ await deleteCompanyItem(id); await load() }
  function onChangeDraft(key: string, value: string){ setDraft(prev=> ({ ...(prev||{}), [key]: value })) }
  async function onSaveEdit(){ if(!editingId||!draft) return; await updateCompanyItem(String(editingId), { data: draft }); setEditingId(null); setDraft(null); await load() }
  ...
  return (
    <>
      ...
      <FullBleed>
        <AutoScale lock={!!editingId} minScale={0.2} deps={[sorted.length, columns.length]} onScale={(k)=>{ try { document.documentElement.style.setProperty('--ui-scale', String(k)); } catch {} }}>
          <LiquidCard title="Companies to reach" actions={
      ...
      <DealFormModal
        ...
        onSave={async (draft)=>{ const { createCompanyItem } = await import('../api/companies'); await createCompanyItem({ data: draft as any }); setOpen(false); await load() }}
      />
      {menu && (
        <div
          ref={menuRef}
