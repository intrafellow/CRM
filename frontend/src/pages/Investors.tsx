...
import { getInvestors, updateInvestorItem, deleteInvestorItem } from '../api/investors'
...
const REQUIRED_HEADERS = ['Investor','Connection','Target ticket','Target sectors','Relevant?','Comments','Discussed fund','Discussed A3','Discussed Lab Vkusa'] as const
...
export default function Investors() {
  ...
  async function onDelete(id: string){ await deleteInvestorItem(id); await load() }
  function onChangeDraft(key: string, value: string){ setDraft(prev=> ({ ...(prev||{}), [key]: value })) }
  async function onSaveEdit(){ if(!editingId||!draft) return; await updateInvestorItem(String(editingId), { data: draft }); setEditingId(null); setDraft(null); await load() }
    ...
    <>
      ...
      <FullBleed>
        <AutoScale lock={!!editingId} deps={[sorted.length, columns.length]} onScale={(k)=>{ try { document.documentElement.style.setProperty('--ui-scale', String(k)); } catch {} }}>
          <LiquidCard title="List of Investors" actions={
      ...
      <DealFormModal
        ...
        onSave={async (draft)=>{ const { createInvestorItem } = await import('../api/investors'); await createInvestorItem({ data: draft as any }); setOpen(false); await load() }}
      />
      {menu && (
        <div
          ref={menuRef}
