...
import { getAdvisors, updateAdvisorItem, deleteAdvisorItem } from '../api/advisors'
...
const REQUIRED_HEADERS = ['Advisor','Contact persons','Type','Comment','Responsible','Date of the last meeting of the responsible person','Months since the last meeting'] as const
...
export default function Advisors() {
  ...
  async function onDelete(id: string){ await deleteAdvisorItem(id); await load() }
  function onChangeDraft(key: string, value: string){ setDraft(prev=> ({ ...(prev||{}), [key]: value })) }
  async function onSaveEdit(){ if(!editingId||!draft) return; await updateAdvisorItem(String(editingId), { data: draft }); setEditingId(null); setDraft(null); await load() }
    ...
    <>
      ...
      <FullBleed>
        <AutoScale lock={!!editingId} deps={[sorted.length, columns.length]} onScale={(k)=>{ try { document.documentElement.style.setProperty('--ui-scale', String(k)); } catch {} }}>
          <LiquidCard title="List of Advisors" actions={
      ...
      <DealFormModal
        ...
        onSave={async (draft)=>{ const { createAdvisorItem } = await import('../api/advisors'); await createAdvisorItem({ data: draft as any }); setOpen(false); await load() }}
      />
      {menu && (
        <div
          ref={menuRef}
